...
class StatisticsService {
  ...
  Future<Map<String, dynamic>> _getEquipmentStats() async {
    try {
      final response = await _supabase.from('equipment').select('qty, available_qty, status');
      ...
      return {
        ...
        'utilizationRate': totalQty > 0 ? ((borrowed / totalQty) * 100).toInt() : 0,
      };
    } catch (e) {
  ...
  Future<Map<String, dynamic>> _getCategoryStats() async {
    try {
      ...
      final categoriesResponse = await _supabase.from('equipment_categories').select('category_id');
